...
@CustomConnector(metaJsonFile = "programsConnector.json")
@ManagedComponent
public class ProgramsDataConnector implements SearchOperations<LdapResponse>, TestConnectionOperations {

    private final Logger log;
    private final ReadOnlyProperties connectionProperties;

    public ProgramsDataConnector(
        Logger log,
    ...
    public TestConnectionResponse testConnection(TestConnectionRequest request) {
        try {
            log.info("Testing connection to Programs API");
            
            // Basic connection test
            String baseUrl = connectionProperties.getProperty("baseUrl", "");
            if (baseUrl.isEmpty()) {
                return TestConnectionResponse.from("ProgramsAPI", false, "Base URL not configured");
            }
            
            return TestConnectionResponse.from("ProgramsAPI", true, "Connection successful");
        } catch (Exception e) {
            log.error("Connection test failed", e);
            return TestConnectionResponse.from("ProgramsAPI", false, e.getMessage());
        }
    }
    ...
    public LdapResponse search(LdapSearchRequest searchRequest) {
        try {
            log.info("Performing search for Programs objects");
            
            String filter = searchRequest.getFilter().toString();
            log.debug("Search filter: " + filter);
            
            // Create mock results for selected objects: programs
            List<Map<String, Object>> results = new ArrayList<>();
            
            // Add sample data for demonstration
            Map<String, Object> sampleResult = new HashMap<>();
            sampleResult.put("id", "1");
            sampleResult.put("name", "Sample Programs Object");
            sampleResult.put("type", "programs");
