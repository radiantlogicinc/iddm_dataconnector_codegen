#!/usr/bin/env python3
"""
Comprehensive MCP Server for Data Connector Development
...
print('=== MCP DATA CONNECTOR SERVER EXECUTED ===', file=sys.stderr)
...
mcp = FastMCP("Data Connector MCP Tools")
...
# Add new global for error patterns
ERROR_PATTERNS = {
    "wrong_package_imports": {
        "patterns": [
            r"import com\.radiantlogic\.sdk\.core\.",
            r"import com\.radiantlogic\.iddd\.",
            r"import com\.radiantlogic\.idddm\.",
            r"import com\.radiantlogic\.openapi\.generated\.",
            r"import com\.radiantlogic\.iddm\.base\.annotation\.",
            r"import com\.radiantlogic\.iddm\.base\.logging\.",
            r"import com\.radiantlogic\.iddm\.operation\.",
            r"import com\.radiantlogic\.iddm\.request\.",
            r"import com\.radiantlogic\.iddm\.response\.",
        ],
        "replacements": [
            "import com.radiantlogic.iddm.",
            "import com.radiantlogic.iddm.",
            "import com.radiantlogic.iddm.",
            "import com.okta.myaccount.myaccountmanagement.",
            "import com.radiantlogic.iddm.annotations.",
            "import com.radiantlogic.iddm.base.",
            "import com.radiantlogic.iddm.base.",
            "import com.radiantlogic.iddm.ldap.",
            "import com.radiantlogic.iddm.ldap.",
        ]
    },
    "wrong_interface_names": {
        "patterns": [
            r"ReadOperations",
            r"ResponseEntity",
            r"ResponseStatus",
            r"SearchRequest",
            r"LdapLdapSearchRequest",
            r"TestConnectionResponse\(.*\)",
            r"LdapResponse<Object>",
            r"getResultCode\(\)",
            r"getData\(\)",
        ],
        "replacements": [
            ...
            "LdapSearchRequest",
            "LdapSearchRequest",
            "TestConnectionResponse.from",
            "LdapResponse<String>",
            "getStatus()",
            "getData()",
        ]
    },
    "wrong_property_names": {
        "patterns": [
            r"InjectableProperties\.CONNECTION_CONFIGURATION",
            r"getBase\(\)",
            r"getBaseDN\(\)",
        ],
        "replacements": [
            "InjectableProperties.CUSTOM_DATASOURCE_PROPERTIES",
            "getFilter()",
            "getFilter()",
        ]
    },
    "missing_utility_classes": [
        "StringUtil",
        "ApiClient",
        "BaseApi"
    ],
    "wrong_constructor_patterns": {
        "patterns": [
            r"new TestConnectionResponse\(.*LdapResultCode\.SUCCESS.*\)",
            r"new TestConnectionResponse\(.*true.*\)",
        ],
        "replacements": [
            "TestConnectionResponse.from(\"OktaMyAccountAPI\", true",
            "TestConnectionResponse.from(\"OktaMyAccountAPI\", true",
        ]
    }
}

# Add labeled few-shot examples for error correction
ERROR_CORRECTION_EXAMPLES = [
    {
        "broken_code": """package com.radiantlogic.custom.dataconnector;

import com.radiantlogic.sdk.core.CustomConnector;
import com.radiantlogic.iddd.ReadOperations;
import com.radiantlogic.idddm.ResponseEntity;

@CustomConnector(metaJsonFile = "testConnector.json")
public class TestDataConnector implements ReadOperations<Object, ResponseEntity> {
    // ... rest of code
}""",
        "fixed_code": """package com.radiantlogic.custom.dataconnector;
...
@CustomConnector(metaJsonFile = "testConnector.json")
public class TestDataConnector implements SearchOperations<LdapSearchRequest, LdapResponse<String>> {
    // ... rest of code
}""",
        "error_type": "wrong_imports_and_interfaces"
    },
    {
        "broken_code": """    @Override
    public TestConnectionResponse testConnection(TestConnectionRequest request) {
        try {
            // ... test logic
            return new TestConnectionResponse(LdapResultCode.SUCCESS, true, "Success");
        } catch (Exception e) {
            return new TestConnectionResponse(LdapResultCode.OTHER, false, e.getMessage());
        }
    }""",
        "fixed_code": """    @Override
    public TestConnectionResponse testConnection(TestConnectionRequest request) {
        try {
            // ... test logic
            return TestConnectionResponse.from("TestAPI", true, "Success");
        } catch (Exception e) {
            return TestConnectionResponse.from("TestAPI", false, e.getMessage());
        }
    }""",
        "error_type": "wrong_constructor_patterns"
    },
    {
        "broken_code": """    @Override
    public LdapResponse<String> search(LdapSearchRequest searchRequest) {
        try {
            String searchTerm = searchRequest.getBase().toString();
            // ... search logic
    ...
    public LdapResponse<String> search(LdapSearchRequest searchRequest) {
        ...
        "error_type": "wrong_method_calls"
    },
    {
        "broken_code": """    public TestDataConnector(
            Logger log,
            @Property(name = InjectableProperties.CONNECTION_CONFIGURATION) ReadOnlyProperties connectionProperties
    ) {
        this.log = log;
        // ... initialization
    }""",
        "fixed_code": """    public TestDataConnector(
            Logger log,
        ...
        "error_type": "wrong_property_names"
    },
    {
        "broken_code": """    @Override
    public LdapResponse<String> search(LdapSearchRequest searchRequest) {
    ...
    public LdapResponse<String> search(LdapSearchRequest searchRequest) {
        ...
        "error_type": "correct_code_no_changes"
    }
]
...
class LargeOpenAPIParser:
    ...
    def _extract_object_from_path(self, path: str) -> str:
        """Extract object name from path with better logic"""
        path_parts = path.strip('/').split('/')
        if len(path_parts) > 0:
            # Try to find meaningful object name
            for part in path_parts:
                if part and not part.startswith('{') and len(part) > 2:
                    return part
            # Fallback to first non-empty part
            return path_parts[0] if path_parts[0] else path_parts[1] if len(path_parts) > 1 else "default"
        return "default"
...
def update_pom_source_directory(java_client_api_dir: str) -> Dict[str, Any]:
    ...
    try:
        ...
        old_source = r'<source>java_client/[^<]+</source>'
        new_source = f'<source>java_client/{client_name}</source>'
        ...
        if f'<source>java_client/{client_name}</source>' in pom_content:
...
# =============================
# OpenAPI Tools (from source_json)
# =============================
...
def initialize_openapi_spec(
    ...
    """Initialize OpenAPI specification and create source/target JSON files"""
    ...
    try:
        ...
        if os.path.exists(source_json_file_path):
            ...
            print(f"Parsing OpenAPI spec and generating source JSON", file=sys.stderr)
        ...
        return {
            "success": True,
            "message": f"OpenAPI spec initialized successfully",
...
@mcp.tool()
def list_objects(json_type: str = "source") -> Dict[str, Any]:
...
def list_object_methods(
    ...
    try:
        ...
        if json_type == "source":
            ...
            # PATCH: Always load latest target.json from disk
            target_file = os.path.join(PROJECT_ROOT, "target.json")
            if os.path.exists(target_file):
                data = load_json_data(target_file)
...
@mcp.tool()
def select_object_for_target(object_name: str) -> Dict[str, Any]:
    ...
    try:
        ...
        # Debug: print available keys and full target_data before
        debug_log_path = os.path.join(PROJECT_ROOT, "select_object_debug.log")
        with open(debug_log_path, 'a') as dbg:
            dbg.write(f"Selecting '{object_name}' from keys: {list(data.keys())}\n")
            dbg.write(f"[BEFORE] target_data: {json.dumps(target_data, indent=2)}\n")
        ...
        if not source_object:
            with open(debug_log_path, 'a') as dbg:
                dbg.write(f"Object '{object_name}' NOT FOUND.\n")
        ...
        # Debug: print full target_data after update
        with open(debug_log_path, 'a') as dbg:
            dbg.write(f"[AFTER] target_data: {json.dumps(target_data, indent=2)}\n")
        save_json_data(target_file, target_data)
        with open(debug_log_path, 'a') as dbg:
            dbg.write(f"Object '{object_name}' added to target. Current objects: {list(target_data['objects'].keys())}\n")
        return {
            "success": True,
        ...
        with open(os.path.join(PROJECT_ROOT, 'select_object_debug.log'), 'a') as dbg:
            dbg.write(f"Exception: {e}\n")
        return {"success": False, "error": str(e)}

@mcp.tool()
def remove_object_from_target(object_name: str) -> Dict[str, Any]:
...
# =============================
# Data Connector Code Generation Tools
# =============================
...
"""
        ...
        for iteration in range(max_iterations):
            ...
            # Save generated code for unified connector
            output_dir = os.path.join(experiment_dir, "src/main/java/com/radiantlogic/custom/dataconnector")
            test_dir = os.path.join(experiment_dir, "src/test/java/com/radiantlogic/custom/dataconnector")
            config_dir = os.path.join(experiment_dir, "src/main/resources/com/radiantlogic/custom/dataconnector")
...
def clear_generated_files() -> Dict[str, Any]:
    ...
    ⚠️  LIMITATION: This function has a known issue with file deletion in the MCP server context.
    The function returns success but may not actually delete files due to MCP server file operation limitations.
    
    If you need to clear generated files, please use the manual command:
    rm -rf src/main/java/com/radiantlogic/custom/dataconnector/*.java src/test/java/com/radiantlogic/custom/dataconnector/*.java target/*.jar
    """
    try:
...
def hello_world(name: str) -> Dict[str, Any]:
    ...
    return {
        "success": True,
        "message": f"Hello, {name}! MCP Data Connector server is running."
    }
